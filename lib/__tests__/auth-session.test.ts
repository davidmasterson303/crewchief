...
import { resolveAuthEvent, loginUrlFor } from '@/lib/auth-session';
import { PUBLIC_DEMO_ROUTES } from '@/lib/demo-contract';
...
describe('an anonymous visitor is never redirected', () => {
  ...
  it('does not redirect a signed-out visitor off a demo vehicle page even mid-session', () => {
    ...
    const decision = resolveAuthEvent({
      ...
      hasSession: false,
      hadSession: true,
      pathname: `/dashboard/${require('@/lib/demo').DEMO_VEHICLE_IDS[0]}`,
    });
