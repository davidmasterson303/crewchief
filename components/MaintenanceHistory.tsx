...
function getCategoryIcon(desc: string, category?: string) {
  ...
  if (d.includes('air') || d.includes('filter') || d.includes('intake') || c.includes('filter')) return { Icon: Wind, color: 'text-cyan-400', bg: 'bg-cyan-400/10 border-cyan-400/20' };
  ...
  return { Icon: Wrench, color: 'text-cyan-400', bg: 'bg-cyan-400/10 border-cyan-400/20' };
}
...
export default function MaintenanceHistory({ vehicleId, documents, lineItems = [], completedServiceItems = [], maintenanceLineItems = [], onUploadComplete, onItemDeleted }: MaintenanceHistoryProps) {
  ...
  const getCategoryBadge = (record: MaintenanceRecord) => {
    ...
    if (record.original_category === 'labor') return <Badge variant="outline" className="bg-cyan-400/10 text-cyan-300 border-cyan-400/25 text-xs font-medium">Labor</Badge>;
          ...
          {availableFilters.length > 0 && (
            <div className="flex gap-1.5 flex-wrap">
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all h-9 ${filterType === null ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-400/30' : 'bg-white/4 text-white/50 border border-white/10 hover:border-white/20 hover:text-white/70'}`}
              >
                All
              </button>
              {availableFilters.map((type) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all h-9 capitalize ${filterType === type ? 'bg-cyan-500/20 text-cyan-300 border border-cyan-400/30' : 'bg-white/4 text-white/50 border border-white/10 hover:border-white/20 hover:text-white/70'}`}
                >
                  {type}
                </button>
              ))}
        ...
        {allRecords.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
            <div className="relative mb-5">
              ...
              <div className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-cyan-400/15 border border-cyan-400/25 flex items-center justify-center">
